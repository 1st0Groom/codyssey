# 1. REPL(Read-Eval-Print Loop) 구조로 미니 깃 구현
# 명령어는 대소문자를 구분하지 않는다. (INIT,init 모두 가능)
# 커밋 메시지나 사용자 이름 등은 공백을 포함할 수 있으며, " "로 묶는다.

# 3. 브랜치(Branch)관리 구현
#    헤드와 브랜치 포인터 개념을 도입하여 Branch, switch 명령어를 구현

# 4. 역색인(inverted index)과 검색기능 구현
#     커밋을 생성할 때마다 해시맵에 단어/작성자 정보를 기록하고(인덱싱) Search 명령어를 구현
#     여기서 역색인이란 -> 이 단어는 어느어느 문서에 있느냐~ 를 정리해둔 것.
#     장단점은 어떤 단어를 엄청 빨리(O(1)) 찾을 수 있다. 단점은 메모리 저장공간이 커진다.
