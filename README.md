...
## 3. 학습 목표 달성 (개념 정리)
*(과제 요구사항에 따라 아래 개념들을 본인의 언어로 간략히 설명하세요)*
- **절대 경로와 상대 경로의 차이:** 절대경로 : 파일시스템의 최상위 경로 / 부터 시작해서 특정 파일까지 도달하는 경로를 기록 , 상대경로 : 현재 내가 위치한 디렉토리를 기준으로 타겟 파일의 위치를 나타내는 경로
- **파일 권한(r/w/x)과 755/644의 의미:** read,write,excute 의 약자로 읽기 권한, 쓰기 권한, 실행권한을 의미하고 4는 r 2는 w 1 는 x 로 세 가지 권한의 합으로 구성됨, 또한 100의자리의 숫자는 유저권한, 10의자리 숫자는 그룹권한, 1의자리 숫자는 그 외의 기타 사용자를 의미함
- **포트 매핑이 필요한 이유:** 포트 포워딩이라고도 하며 외부 네트워크에서 내부 네트워크상 기기에 접속할 수 있도록 통로를 열어주는 기술, 포트는 ip 주소가 아파트 단지 주소라면, 포트 번호는 동/호수와 같다고 생각하면 편함.
- **Docker 볼륨(영속 데이터)의 개념:** 도커는 컨테이너를 삭제하면 그 안에서 생성된 모든 데이터가 소멸함.
  이를 해결하기 위한 개념이 데이터 영속성이며 도커에선 주로 볼륨을 통해 구현함. 동작 원리는 호스트 os의 특정 디렉토리를 컨테이너 내부의 디렉토리와 연결(mount)하는 방식으로 사용, 데이터 영속성에는 볼륨이외에도 bind mount, tmpfs mount 로도 구현할 수 있음 
- **Git과 GitHub의 역할 차이:** git 은 소스코드 변경 이력을 관리하는 분산 버전 관리 시스템 소프트웨어이고 내 컴퓨터(로컬)에 설치하는 프로그램. 파일 버전기록, 롤백, 브렌치를 나눠 협업할 수 있음. 로컬환경에서 구축했기 때문에 인터넷 연결이 필요없음. 
github는 웹사이트(클라우드)이고 내 컴퓨터에 있는 git 저장소를 인터넷상에 올려서 다른 사람들과 공유하고 협업할 수 있음.
git의 기능을 웹ui 로 편리하게 제공함.
...
```bash
$mkdir -p ~/workspace/codyssey$ cd ~/workspace/codyssey
$ mkdir src
...
### 4.2. git 설정 및 초기화
$ git init
$git config user.name "camus"$ git config user.email "nadomolla08@naver.com"
$git config init.defaultBranch main$ git config --list
...
Bash
$echo "<h1>Hello Codyssey! This is my custom Nginx.</h1>" > src/index.html$ docker build -t my-web:1.0 .
...
$echo "<h1>Bind Mount is LIVE!</h1>" > src/index.html$ curl http://localhost:8080
...
Bash
$docker volume create my-data$ docker run -d --name vol-test -v my-data:/app ubuntu sleep infinity
$docker exec vol-test sh -c "echo 'Persistence Verified' > /app/result.txt"$ docker rm -f vol-test
...
확인 및 해결: Ctrl + C를 입력하여 대기 상태를 강제 취소한 후, 역슬래시 없이 한 줄로 명령어를 재입력(또는 올바르게 줄 바꿈하여 입력)하여 정상 실행함.
...
원인 가설: 브라우저 개발자 도구(F12)의 네트워크 탭 분석 결과, 서버 응답이 304 Not Modified로 나타남. 즉, 서버는 정상이지만 브라우저가 변경된 서버의 데이터를 받아오지 않고 로컬의 잘못된 구형 캐시를 렌더링하고 있다고 진단.
